...
class EstudianteStats extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        return [
            Stat::make('Mis Puntos Totales', intval($puntosTotal ?: 0))
                ->description('Puntos acumulados')
                ->descriptionIcon('heroicon-m-star')
                ->color('warning'),
            
            Stat::make('Registros Este Mes', intval($registrosMes ?: 0))
                ->description('Actividad del mes actual')
                ->descriptionIcon('heroicon-m-calendar')
                ->color('success'),
            
            Stat::make('Total Reciclado', intval($totalMateriales ?: 0) . ' kg')
